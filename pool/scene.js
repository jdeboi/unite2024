...
function init() {
  ...
  effect.colorMatrixLeft = new THREE.Matrix3().fromArray([
    0.437, -0.062, -0.048, 0.449, -0.062, -0.05, 0.164, -0.024, -0.017,
  ]);

  // Color matrix for the right input image
  effect.colorMatrixRight = new THREE.Matrix3().fromArray([
...
const effectController = {
  ...
  distortionScale: 0.7,
  size: 4.0,
  waterColor: "#ff0000", // Default water color
};
...
function setSky() {
  ...
  function guiChanged() {
    ...
    waterUniforms["distortionScale"].value = effectController.distortionScale;
    waterUniforms["size"].value = effectController.size;
    waterUniforms["waterColor"].value.set(effectController.waterColor);
  ...
  folderWater
    .add(effectController, "distortionScale", 0, 8, 0.1)
    .onChange(guiChanged);
  folderWater.add(effectController, "size", 0.1, 10, 0.1).onChange(guiChanged);
  folderWater.addColor(effectController, "waterColor").onChange(guiChanged); // Color picker
